...
export default function getConfig(overrideConfig) {
	...
	if (!process.env.OAUTH_ACCESS_URL && process.env.ANONYMOUS_ACCESS_URL) {
		console.warn(
			'The ANONYMOUS_ACCESS_URL env variable is deprecated - please rename to OAUTH_ACCESS_URL'
		);
	}
	if (!process.env.OAUTH_AUTH_URL && process.env.ANONYMOUS_AUTH_URL) {
		console.warn(
			'The ANONYMOUS_AUTH_URL env variable is deprecated - please rename to OAUTH_AUTH_URL'
		);
	}
	const config = {
	...
	return Promise.resolve({ ...config, ...overrideConfig }).then(validateConfig);
}
...
function validateConfig(config) {
	...
	const configSchema = Joi.object()
		.keys({
			...
			COOKIE_ENCRYPT_SECRET: Joi.string()
				.min(32)
				.required()
				.error(
					new Error(
			...
			PHOTO_SCALER_SALT: Joi.string()
				.min(1)
				.required()
				.error(new Error('get PHOTO_SCALER_SALT from web platform team')),
			oauth: Joi.object()
				.keys({
